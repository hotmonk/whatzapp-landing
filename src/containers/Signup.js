...
function Signup() {
  ...
  const postrequesthandler = async (body) => {
    ...
    const response = await fetch(
      "https://eazybe.com/api/v1/whatzapp/basicInfo",
      requestOptions
    );
  ...
  const onSubmitHandler = (e) => {
    ...
    else strmonth = (future.getMonth() + 1).toString();

    var strdate;
    if (future.getDate() <= 9) strdate = "0" + future.getDate().toString();
    else strdate = future.getDate().toString();
    ...
    // axios.post("https://eazybe.com/api/v1/whatzapp/basicInfo", jsonbody)
